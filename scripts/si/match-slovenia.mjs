// Match Slovenia GZS federation data against DB courses.
//
// Run: node --env-file=.env.local scripts/si/match-slovenia.mjs

import { readFileSync, writeFileSync } from 'node:fs'
import { createClient } from '@supabase/supabase-js'

const FED_PATH = 'scripts/si/si-clubs-gzs.json'
const REPORT_PATH = 'scripts/si/slovenia-match-report.md'
const CANDIDATES_PATH = 'scripts/si/slovenia-match-candidates.json'
...
const SI_STOPWORDS = [
  'gk', 'gc', 'gck', 'g&ck', 'g', 'golf', 'klub', 'club', 'country',
  'goriski', 'koroski', // diacritics-stripped versions
  'sl', 'slovenija', 'd.o.o', 'doo', 'sa',
  'i', 'ii', 'iii', 'iv', 'v',
  'resort', 'park',
]
...
const norm = (s) =>
  (s || '')
    ...
    .normalize('NFD')
    .replace(/[̀-ͯ]/g, '')
    .replace(new RegExp(`\\b(${SI_STOPWORDS.join('|')})\\b`, 'g'), '')
    .replace(/[^a-z0-9]+/g, ' ')
    .trim()
...
async function fetchSloveniaCourses() {
  const PAGE = 1000
  const all = []
...
async function main() {
  console.log('Loading GZS federation data…')
  const fed = JSON.parse(readFileSync(FED_PATH, 'utf8'))
  const fedKlubs = fed.clubs
  console.log(`  ${fedKlubs.length} GZS klubs`)

  console.log('Loading DB courses (SI)…')
  const db = await fetchSloveniaCourses()
  ...
  for (const bucket of ['high', 'medium', 'low', 'dedup_lost', 'no_match']) {
    ...
    lines.push('')
    lines.push('| DB club | rows | GZS name | sim | boost | website | email | phone | reasons |')
    lines.push('|---|---|---|---|---|---|---|---|---|')
  ...
  if (unmatchedFed.length) {
    lines.push('## Unmatched GZS klubs')
    lines.push('')
  ...
  console.log(`  Unmatched GZS: ${unmatchedFed.length}`)
}
