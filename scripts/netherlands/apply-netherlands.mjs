// Generic apply script for Netherlands match candidates.
// Modeled efter scripts/england/apply-england.mjs — bucket-baseret, idempotent.
//
// Args:
//   --bucket=<high|medium|low>   bucket at processere (required)
//   --apply                      write changes (default = dry run)
//   --skip="Club A,Club B"       skip specifik klubber
//   --label=<name>               log file suffix (default = bucket)
//
// Examples:
//   node --env-file=.env.local scripts/netherlands/apply-netherlands.mjs --bucket=high
//   node --env-file=.env.local scripts/netherlands/apply-netherlands.mjs --bucket=high --apply
//   node --env-file=.env.local scripts/netherlands/apply-netherlands.mjs --bucket=medium --apply --skip="Foo,Bar"
...
const LOG_PATH = `scripts/netherlands/apply-netherlands-${label}-log.json`
