// Apply GR Pass 2a — federation-driven kontakt fra HGF.
//
// Source: scripts/gr/greece-match-candidates.json
// Targets: bucket=high → fill-NULL website/email/phone (COALESCE-only).
//
// Run:
//   node --env-file=.env.local scripts/gr/apply-slovenia-pass2a.mjs            (dry)
//   CONFIRM=ja node --env-file=.env.local scripts/gr/apply-slovenia-pass2a.mjs --apply
...
const BACKUP_PATH = `scripts/gr/courses-backup-gr-pass2a-${today}.json`
const LOG_PATH = `scripts/gr/apply-gr-pass2a-log.json`
...
async function backup() {
  console.log('Pre-apply backup of GR rows…')
  const PAGE = 1000
  const all = []
