// Apply Wales coord fixes from audit. Same pattern as Ireland's apply-coords.
// Default: only flagLevel='high' (OSM+WG <200m + DB far).
// With --include-medium-consensus: adds medium where both sources >500m off
// AND OSM↔WG <500m apart.
//
// Run dry-run:
//   node --env-file=.env.local scripts/wales/apply-wales-coords.mjs
// Apply with medium-consensus:
//   node --env-file=.env.local scripts/wales/apply-wales-coords.mjs --apply --include-medium-consensus

import { readFileSync, writeFileSync } from 'node:fs'
import { createClient } from '@supabase/supabase-js'

const AUDIT_PATH = 'scripts/wales/wales-coords-audit.json'
const LOG_PATH = 'scripts/wales/apply-wales-coords-log.json'
...
for (const e of targets) {
  ...
  console.log(`  → New:  ${lat}, ${lon}  (OSM=${e.osm.distFromDb}m, WG=${e.sg.distFromDb}m off; OSM↔WG=${e.osmSgDist}m)`)
