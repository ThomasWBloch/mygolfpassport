// Generic apply script for England match candidates.
// Modeled on scripts/ireland/apply-ireland.mjs — reusable across high/medium/low buckets,
// idempotent (skips rows already at proposed value).
//
// Args:
...
//
// Examples:
//   node --env-file=.env.local scripts/england/apply-england.mjs --bucket=high
//   node --env-file=.env.local scripts/england/apply-england.mjs --bucket=high --apply
//   node --env-file=.env.local scripts/england/apply-england.mjs --bucket=medium --apply --skip="Foo,Bar"
...
const LOG_PATH = `scripts/england/apply-england-${label}-log.json`
