...
const clean = JSON.parse(readFileSync('scripts/norwegian-club-addresses-cleaned.json', 'utf8'))
const review = JSON.parse(readFileSync('scripts/norwegian-club-addresses-review.json', 'utf8'))
...
const JUNK_TOKENS = [
  'åpningstider', 'telefontid', 'kontoret', 'nødnummer', 'norway', 'logo',
  'velkommen', 'informasjon', 'snarveier', 'hurtigmeny', 'resepsjon',
  'kontakt', 'medlem', 'copyright', 'made by', 'powered by',
  'telefon', 'e-post', 'epost', 'kommunikasjon', 'facebook', 'instagram',
  'personvern', 'cookies', 'informasjonskapsler', 'administrasjon',
  'proshop', 'klubben', 'klubbhuset', 'simulator', 'årsmøte',
  'påmelding', 'fasiliteter', 'nytt ', 'welcome',
]
...
function addrIsSafe(addr) {
  ...
  return /\b\d{4}\b/.test(addr)  // Norwegian postal code
}
...
while (true) {
  const { data, error } = await s.from('courses')
    ...
    .eq('country', 'Norway').range(from, from + 999)
...
console.log(`DB rows (Norway): ${all.length}`)
