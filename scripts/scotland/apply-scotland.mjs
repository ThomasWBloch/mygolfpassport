// Generic apply script for Scotland match candidates (Pass 2 — websites/addresses).
// Modeled on apply-england.mjs.
//
// Args:
//   --bucket=<high|medium|low>   which bucket to process (required)
//   --apply                      write changes (default = dry run)
//   --skip="Club A,Club B"
//   --label=<name>
//
// Run: node --env-file=.env.local scripts/scotland/apply-scotland.mjs --bucket=high
...
const LOG_PATH = `scripts/scotland/apply-scotland-${label}-log.json`
...
for (const e of entries) {
  ...
  const deltaSummary = Object.entries(delta)
    ...
    .join(', ')
  console.log(`${e.club} (${rows.length} courses): ${deltaSummary}`)
  ...
  log.push({
    club: e.club, country: e.country, courseCount: rows.length,
    delta, fullProposal: e.proposedUpdate, sources: e.updateSources,
