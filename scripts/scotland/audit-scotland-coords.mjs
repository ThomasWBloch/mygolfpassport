// Coords audit for Scotland: compare DB coords vs OSM (and SG if available).
// Same logic as Ireland's audit. Falls back to OSM-only if SG fetch failed.
//
// Output:
//   scripts/scotland/scotland-coords-audit.json
//   scripts/scotland/scotland-coords-audit.md
//
// Run: node scripts/scotland/audit-scotland-coords.mjs

import { readFileSync, writeFileSync, existsSync } from 'node:fs'

const today = new Date().toISOString().slice(0, 10)
const BACKUP_PATH = `scripts/scotland/courses-backup-scotland-${today}.json`
const OSM_PATH = 'scripts/scotland/scotland-clubs-osm.json'
const SG_PATH = 'scripts/scotland/scotland-sg-clubs.json'
const JSON_OUT = 'scripts/scotland/scotland-coords-audit.json'
const MD_OUT = 'scripts/scotland/scotland-coords-audit.md'
...
const hasSG = existsSync(SG_PATH)
const sg = hasSG ? JSON.parse(readFileSync(SG_PATH, 'utf8')) : []

console.log(`DB: ${db.length}, OSM: ${osm.length}, SG: ${hasSG ? sg.length : 'NOT AVAILABLE — single-source audit'}`)
...
for (const [club, courses] of dbByClub) {
  ...
  const sgHit = hasSG ? bestNameMatch(rep.club, sg, (c) => c.latitude, (c) => c.longitude) : null
  ...
  if (osmDist != null && sgDist != null) {
    if (osmSgDist < 200 && (osmDist > 500 || sgDist > 500)) {
      level = 'high'
      reason = `OSM+SG consensus (${Math.round(osmSgDist)}m apart), DB diverges (osm=${Math.round(osmDist)}m, sg=${Math.round(sgDist)}m)`
      ...
      reason = osmSgDist > 500
        ...
        : `Both >500m off, OSM↔SG=${Math.round(osmSgDist)}m`
    ...
    reason = `SG-only and >1km off (${Math.round(sgDist)}m). Needs second-source verification.`
  }
...
writeFileSync(JSON_OUT, JSON.stringify({ flagged, okCount: okClubs.length, noSourceMatch, hasSG }, null, 2))

const md = []
md.push('# Scotland coordinate audit')
md.push(`Generated: ${new Date().toISOString().slice(0, 19)}`)
md.push(hasSG ? '' : '(SG fetch unavailable — single-source audit. Auto-fix not safe.)')
md.push('')
md.push('## Summary')
md.push('')
...
const renderEntry = (e) => {
  ...
  if (e.sg)  lines.push(`- SG:  lat=${e.sg.lat}, lon=${e.sg.lon} (sim=${e.sg.sim}, ${e.sg.distFromDb}m from DB)`)
  if (e.osmSgDist != null) lines.push(`- OSM↔SG distance: ${e.osmSgDist}m`)
  lines.push(`- **Reason:** ${e.flagReason}`)
  if (e.flagLevel === 'high' && e.osm && e.sg) {
    const lat = (e.osm.lat + e.sg.lat) / 2
    const lon = (e.osm.lon + e.sg.lon) / 2
    lines.push(`- Suggested fix: lat=${lat.toFixed(7)}, lon=${lon.toFixed(7)} (OSM+SG midpoint)`)
  }
  lines.push('')
  return lines.join('\n')
}
