// Apply Scotland coord fixes from audit. Same pattern as Ireland's apply-coords.
// Default: only flagLevel='high' (OSM+SG <200m + DB far).
// With --include-medium-consensus: adds medium where both sources >500m off
// AND OSM↔SG <500m apart.
//
// Run dry-run:
//   node --env-file=.env.local scripts/scotland/apply-scotland-coords.mjs
// Apply with medium-consensus:
//   node --env-file=.env.local scripts/scotland/apply-scotland-coords.mjs --apply --include-medium-consensus

import { readFileSync, writeFileSync } from 'node:fs'
import { createClient } from '@supabase/supabase-js'

const AUDIT_PATH = 'scripts/scotland/scotland-coords-audit.json'
const LOG_PATH = 'scripts/scotland/apply-scotland-coords-log.json'
...
for (const e of targets) {
  ...
  console.log(`  → New:  ${lat}, ${lon}  (OSM=${e.osm.distFromDb}m, SG=${e.sg.distFromDb}m off; OSM↔SG=${e.osmSgDist}m)`)
