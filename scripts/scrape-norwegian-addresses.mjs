...
const OUT_FILE = 'scripts/norwegian-club-addresses.json'
const URLS = JSON.parse(readFileSync('scripts/norwegian-club-websites.json', 'utf8'))
const CONCURRENCY = 5
...
// Norwegian: 4-digit postal codes (no space), streets often end in -vei/-veien/-gate/-gaten
const POSTAL_RE = /\b(\d{4})\s+([A-ZÆØÅ][a-zæøåA-ZÆØÅ\-\.\s]{2,40})/
const PHONE_RE = /(?:\+47[\s\-]?|\b)\d{2}[\s\-]?\d{2}[\s\-]?\d{2}[\s\-]?\d{2}\b/
const STREET_HINT_RE = /\b[A-ZÆØÅ][a-zæøå][a-zæøåA-ZÆØÅ\- ]{2,40}(?:veien|vegen|vei|veg|gaten|gata|gate|stien|plassen|plass|torget)\b[\s\w\-]*?\d+[A-Za-z]?/
...
function extractFromText(text) {
  ...
  if (postalMatch) {
    const postal = `${postalMatch[1]} ${postalMatch[2].split(/\s+/).slice(0, 3).join(' ').replace(/[,\.].*$/, '').trim()}`
    ...
    if (streetLine) {
      const streetMatch = streetLine.match(STREET_HINT_RE) || streetLine.match(/[A-ZÆØÅ][a-zæøåA-ZÆØÅ\- ]{3,40}\s\d+[A-Z]?/)
      if (streetMatch) {
