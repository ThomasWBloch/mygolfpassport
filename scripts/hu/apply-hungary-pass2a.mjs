// Apply HU Pass 2a — federation-driven kontakt fra MGSZ.
//
// Source: scripts/hu/hungary-match-candidates.json
// Targets: bucket=high → fill-NULL website/email/phone (COALESCE-only).
//
// Run:
//   node --env-file=.env.local scripts/hu/apply-slovenia-pass2a.mjs            (dry)
//   CONFIRM=ja node --env-file=.env.local scripts/hu/apply-slovenia-pass2a.mjs --apply
...
const BACKUP_PATH = `scripts/hu/courses-backup-hu-pass2a-${today}.json`
const LOG_PATH = `scripts/hu/apply-hu-pass2a-log.json`
...
async function backup() {
  console.log('Pre-apply backup of HU rows…')
  const PAGE = 1000
  const all = []
