// Match Hungary MGSZ federation data against DB courses.
//
// Run: node --env-file=.env.local scripts/hu/match-slovenia.mjs

import { readFileSync, writeFileSync } from 'node:fs'
import { createClient } from '@supabase/supabase-js'

const FED_PATH = 'scripts/hu/hu-clubs-mgsz.json'
const REPORT_PATH = 'scripts/hu/hungary-match-report.md'
const CANDIDATES_PATH = 'scripts/hu/hungary-match-candidates.json'
...
const HU_STOPWORDS = [
  'golf', 'klub', 'club', 'country', 'gc', 'cc',
  'magyar', 'hungary', 'hungarian', 'budapest',
  'kft', 'zrt', 'bt', 'sa',
  'i', 'ii', 'iii', 'iv', 'v',
  'resort', 'park', 'academy',
  'hotel',
...
const norm = (s) =>
  (s || '')
    ...
    .normalize('NFD')
    .replace(/[̀-ͯ]/g, '')
    .replace(new RegExp(`\\b(${HU_STOPWORDS.join('|')})\\b`, 'g'), '')
    .replace(/[^a-z0-9]+/g, ' ')
    .trim()
...
async function fetchHungaryCourses() {
  const PAGE = 1000
  const all = []
...
async function main() {
  console.log('Loading MGSZ federation data…')
  const fed = JSON.parse(readFileSync(FED_PATH, 'utf8'))
  const fedKlubs = fed.clubs
  console.log(`  ${fedKlubs.length} MGSZ klubs`)

  console.log('Loading DB courses (SI)…')
  const db = await fetchHungaryCourses()
  ...
  for (const bucket of ['high', 'medium', 'low', 'dedup_lost', 'no_match']) {
    ...
    lines.push('')
    lines.push('| DB club | rows | MGSZ name | sim | boost | website | email | phone | reasons |')
    lines.push('|---|---|---|---|---|---|---|---|---|')
  ...
  if (unmatchedFed.length) {
    lines.push('## Unmatched MGSZ klubs')
    lines.push('')
  ...
  console.log(`  Unmatched MGSZ: ${unmatchedFed.length}`)
}
