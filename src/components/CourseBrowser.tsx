...
export default function CourseBrowser({ countries, playedIds, hiddenIds = [], mode = 'browse', onSelectCourse, userHomeCountry = null, courseCount = 0 }: Props) {
  ...
  // NOTE: same-country namesakes (Eagle Ridge GC across 6 US states) still
  // bundle into ONE card here — that limitation is intentional until the
  // clubs-table migration lands. A previous attempt at appending state
  // suffixes was reverted because the first-row state isn't deterministic
  // across queries and ended up labelling the bundle with whatever state
  // arrived first from Supabase. See project_clubs_table_deferred memory.
