...
function FlyToContinent({ bounds }: { bounds: LatLngBoundsExpression | null }) {
  const map = useMap()
  useEffect(() => {
    if (bounds) {
...
function makeIcon(count: number): L.DivIcon {
  const size = count >= 100 ? 48 : count >= 25 ? 42 : count >= 10 ? 38 : 32
...
export default function AtlasWorldMap({
  ...
  const useInitialFit =
    activeContinent === null && bounds === null && countries.length > 0
  ...
  return (
    ...
    >
      ...
      >
        ...
        {useInitialFit && (
          <FitBounds
            points={countries.map((c) => [c.lat, c.lng] as [number, number])}
          />
        )}
        {!useInitialFit && <FlyToContinent bounds={bounds} />}
            ...
            <Popup>
              ...
              >
                  ...
                  {c.courses.slice(0, 5).map((course, i) => {
                    ...
                    const secondaryLabel =
                      course.club && course.club !== course.name
                        ...
                        : null
                    const fullStars =
                      course.rating != null ? Math.round(course.rating) : 0
                    return (
                      <div key={i}>
                        <div
                          style={{
                        ...
                        {course.rating != null && (
                          ...
                          >
                            <span style={{ color: 'var(--color-mgp-gold-dark)' }}>
                              {'★'.repeat(fullStars)}
                            </span>
                            <span
                              style={{ color: 'var(--color-mgp-border-faint)' }}
                            >
                              {'★'.repeat(5 - fullStars)}
                            </span>
                          </div>
                ...
                {c.count > 5 && (
                  ...
                  >
                    See all {c.count.toLocaleString('en-US')} courses in{' '}
                    {c.country} →
                  </a>
                )}
              </div>
      ...
      >
        {totalRounds.toLocaleString('en-US')}{' '}
        {totalRounds === 1 ? 'course' : 'courses'} ·{' '}
