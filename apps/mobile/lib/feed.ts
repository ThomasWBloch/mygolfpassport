...
/**
 * Ported near-verbatim from apps/web/src/lib/feed.ts — pure Supabase-query
 * logic with no web-specific dependencies, so it needed no real changes
 * (same pattern as the badge-criteria logic ported to the delete-round
 * Edge Function).
 *
 * Pulls together friends' recent rounds, badges, and new friendships into a
 * single chronologically-sorted stream. Empty-state fallback: when the user
 * has no friends yet, we surface their own recent rounds so the feed never
 * looks barren.
 */

export type FeedItem = FeedRoundItem | FeedBadgeItem | FeedFriendshipItem;
...
export async function fetchFeed(
  ...
  const friendshipRowsFiltered = friendRowsDedup.filter(
    (f) => (f.user_id as string) !== userId && (f.friend_id as string) !== userId
  );
  ...
  const merged: FeedItem[] = [...roundItems, ...badgeItems, ...friendshipItems].sort(
    (a, b) => +new Date(b.timestamp) - +new Date(a.timestamp)
  );

  const filtered = before ? merged.filter((i) => +new Date(i.timestamp) < +new Date(before)) : merged;
