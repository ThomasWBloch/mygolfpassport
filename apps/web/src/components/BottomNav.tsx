...
const TABS: Tab[] = [
  {
    // Courses tab links straight to the Atlas view. The home route ('/')
    // is intentionally NOT in matchPrefixes: on the home page no tab is
    // highlighted, since Home is its own surface, not the Courses scope.
    href: '/courses',
    label: 'Courses',
  ...
  },
  {
    href: '/you',
    label: 'You',
    matchPrefixes: ['/you', '/profile', '/badges'],
    Icon: YouIcon,
  },
]
...
function BottomNavInner({ unreadCount }: { unreadCount: number }) {
  ...
  if (ROUTES_WITHOUT_NAV.has(pathname) || isInviteLanding || isChatThread || isLegalPage) return null
