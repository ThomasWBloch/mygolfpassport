// Apply USA Pass 2a — federation-driven kontakt fra OpenGolfAPI.
// Source: scripts/usa/usa-match-candidates.json
// Targets: bucket=high → fill-NULL website + phone (NO email — OGA har ikke email).
//
// Run:
//   node --env-file=.env.local scripts/usa/apply-usa-pass2a.mjs            (dry)
//   CONFIRM=ja node --env-file=.env.local scripts/usa/apply-usa-pass2a.mjs --apply
...
const BACKUP_PATH = `scripts/usa/courses-backup-usa-pass2a-${today}.json`
const LOG_PATH = `scripts/usa/apply-usa-pass2a-log.json`
...
async function backup() {
  console.log('Pre-apply backup of USA rows…')
  const PAGE = 1000
  const all = []
  for (let from = 0; ; from += PAGE) {
    const { data, error } = await sb
      .from('courses')
      .select('id, club, name, latitude, longitude, website, phone, email, holes')
      .eq('country', 'USA')
...
function planUpdate(dbRow, fed) {
  ...
  // Only website + phone for USA (no email in OGA)
...
async function main() {
  ...
  for (const cand of targets) {
    ...
    if (!fed.website && !fed.phone) continue
    for (const dbRow of cand.dbRows) {
      rowCt++
      const upd = planUpdate(dbRow, fed)
      if (upd) plan.push({ klub: cand.dbClub, oga_name: cand.best.name, ...upd })
    }
  }
  console.log(`Planned row-updates: ${plan.length} / ${rowCt} candidate-rows`)

  const counts = { website: 0, phone: 0 }
  for (const p of plan) for (const k of Object.keys(p.set)) counts[k]++
  console.log('  fields:', JSON.stringify(counts))

  if (!APPLY) {
    writeFileSync(LOG_PATH, JSON.stringify({ dryRun: true, plan: plan.slice(0, 100), planSize: plan.length }, null, 2), 'utf8')
  ...
  writeFileSync(LOG_PATH, JSON.stringify({ applied: true, ok, fail, results: results.slice(-200) }, null, 2), 'utf8')
