// Match Greece HGF federation data against DB courses.
//
// Run: node --env-file=.env.local scripts/gr/match-slovenia.mjs

import { readFileSync, writeFileSync } from 'node:fs'
import { createClient } from '@supabase/supabase-js'

const FED_PATH = 'scripts/gr/gr-clubs-hgf.json'
const REPORT_PATH = 'scripts/gr/greece-match-report.md'
const CANDIDATES_PATH = 'scripts/gr/greece-match-candidates.json'
...
const GR_STOPWORDS = [
  'golf', 'club', 'country', 'gc', 'cc', 'hellenic',
  'greece', 'athens', 'athletic',
  'i', 'ii', 'iii', 'iv', 'v',
  'resort', 'park', 'academy', 'association', 'senior',
  'of',
]
...
const norm = (s) =>
  (s || '')
    ...
    .normalize('NFD')
    .replace(/[̀-ͯ]/g, '')
    .replace(new RegExp(`\\b(${GR_STOPWORDS.join('|')})\\b`, 'g'), '')
    .replace(/[^a-z0-9]+/g, ' ')
    .trim()
...
async function fetchGreeceCourses() {
  const PAGE = 1000
  const all = []
...
async function main() {
  console.log('Loading HGF federation data…')
  const fed = JSON.parse(readFileSync(FED_PATH, 'utf8'))
  const fedKlubs = fed.clubs
  console.log(`  ${fedKlubs.length} HGF klubs`)

  console.log('Loading DB courses (Greece)…')
  const db = await fetchGreeceCourses()
  ...
  for (const bucket of ['high', 'medium', 'low', 'dedup_lost', 'no_match']) {
    ...
    lines.push('')
    lines.push('| DB club | rows | HGF name | sim | boost | website | email | phone | reasons |')
    lines.push('|---|---|---|---|---|---|---|---|---|')
  ...
  if (unmatchedFed.length) {
    lines.push('## Unmatched HGF klubs')
    lines.push('')
  ...
  console.log(`  Unmatched HGF: ${unmatchedFed.length}`)
}
