// Match Turkey TGF federation data against DB courses.
//
// Run: node --env-file=.env.local scripts/tr/match-slovenia.mjs

import { readFileSync, writeFileSync } from 'node:fs'
import { createClient } from '@supabase/supabase-js'

const FED_PATH = 'scripts/tr/tr-clubs-tgf.json'
const REPORT_PATH = 'scripts/tr/turkey-match-report.md'
...
const TR_STOPWORDS = [
  'golf', 'klub', 'kulubu', 'kulübü', 'kulup', 'kulüp', 'spor', 'club', 'country', 'gc',
  'turkiye', 'türkiye', 'turkey', 'derne', 'dernegi', 'derneği',
  'i', 'ii', 'iii', 'iv', 'v',
  'resort', 'park', 'ihtisas', 'spor',
]
...
const norm = (s) =>
  (s || '')
    ...
    .normalize('NFD')
    .replace(/[̀-ͯ]/g, '')
    .replace(new RegExp(`\\b(${TR_STOPWORDS.join('|')})\\b`, 'g'), '')
    .replace(/[^a-z0-9]+/g, ' ')
    .trim()
...
async function fetchTurkeyCourses() {
  const PAGE = 1000
  const all = []
...
async function main() {
  console.log('Loading TGF federation data…')
  const fed = JSON.parse(readFileSync(FED_PATH, 'utf8'))
  const fedKlubs = fed.clubs
  console.log(`  ${fedKlubs.length} TGF klubs`)

  console.log('Loading DB courses (Turkey)…')
  const db = await fetchTurkeyCourses()
  ...
  for (const bucket of ['high', 'medium', 'low', 'dedup_lost', 'no_match']) {
    ...
    lines.push('')
    lines.push('| DB club | rows | TGF name | sim | boost | website | email | phone | reasons |')
    lines.push('|---|---|---|---|---|---|---|---|---|')
  ...
  if (unmatchedFed.length) {
    lines.push('## Unmatched TGF klubs')
    lines.push('')
  ...
  console.log(`  Unmatched TGF: ${unmatchedFed.length}`)
}
