// Apply Belgium Pass 2a: website + email + phone enrichment from federation (golf.be).
// Run: node --env-file=.env.local scripts/belgium/apply-belgium-pass2a.mjs --tier=both [--apply]
...
const logPathFor = (tier) => `scripts/belgium/apply-belgium-pass2a-${tier}-log.json`
const backupPathFor = (tier, date) =>
  `scripts/belgium/courses-backup-belgium-pass2a-${tier}-${date}.json`
...
console.log('=== Step 1/3: Backup alle Belgium-rows ===')
const backupRows = []
const PAGE = 1000
...
console.log(`Fetched ${backupRows.length} Belgium-rows fra DB`)

if (apply) {
  writeFileSync(backupPath, JSON.stringify({
    country: 'Belgium', backedUpAt: new Date().toISOString(),
