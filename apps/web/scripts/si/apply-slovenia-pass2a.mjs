// Apply SI Pass 2a — federation-driven kontakt fra GZS.
//
// Source: scripts/si/slovenia-match-candidates.json
// Targets: bucket=high → fill-NULL website/email/phone (COALESCE-only).
//
// Run:
//   node --env-file=.env.local scripts/si/apply-slovenia-pass2a.mjs            (dry)
//   CONFIRM=ja node --env-file=.env.local scripts/si/apply-slovenia-pass2a.mjs --apply
...
const BACKUP_PATH = `scripts/si/courses-backup-si-pass2a-${today}.json`
const LOG_PATH = `scripts/si/apply-si-pass2a-log.json`
...
async function backup() {
  console.log('Pre-apply backup of SI rows…')
  const PAGE = 1000
  const all = []
