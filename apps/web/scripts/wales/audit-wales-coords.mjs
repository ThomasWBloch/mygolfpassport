// Coords audit for Wales: compare DB coords vs OSM (and WG if available).
// Same logic as Ireland's audit. Falls back to OSM-only if WG fetch failed.
//
// Output:
//   scripts/wales/wales-coords-audit.json
//   scripts/wales/wales-coords-audit.md
//
// Run: node scripts/wales/audit-wales-coords.mjs

import { readFileSync, writeFileSync, existsSync } from 'node:fs'

const today = new Date().toISOString().slice(0, 10)
const BACKUP_PATH = `scripts/wales/courses-backup-wales-${today}.json`
const OSM_PATH = 'scripts/wales/wales-clubs-osm.json'
const WG_PATH = 'scripts/wales/wales-wg-clubs.json'
const JSON_OUT = 'scripts/wales/wales-coords-audit.json'
const MD_OUT = 'scripts/wales/wales-coords-audit.md'
...
const hasWG = existsSync(WG_PATH)
const sg = hasWG ? JSON.parse(readFileSync(WG_PATH, 'utf8')) : []

console.log(`DB: ${db.length}, OSM: ${osm.length}, WG: ${hasWG ? sg.length : 'NOT AVAILABLE — single-source audit'}`)
...
for (const [club, courses] of dbByClub) {
  ...
  const sgHit = hasWG ? bestNameMatch(rep.club, sg, (c) => c.latitude, (c) => c.longitude) : null
  ...
  if (osmDist != null && sgDist != null) {
    if (osmSgDist < 200 && (osmDist > 500 || sgDist > 500)) {
      level = 'high'
      reason = `OSM+WG consensus (${Math.round(osmSgDist)}m apart), DB diverges (osm=${Math.round(osmDist)}m, sg=${Math.round(sgDist)}m)`
      ...
      reason = osmSgDist > 500
        ...
        : `Both >500m off, OSM↔WG=${Math.round(osmSgDist)}m`
    ...
    reason = `WG-only and >1km off (${Math.round(sgDist)}m). Needs second-source verification.`
  }
...
writeFileSync(JSON_OUT, JSON.stringify({ flagged, okCount: okClubs.length, noSourceMatch, hasWG }, null, 2))

const md = []
md.push('# Wales coordinate audit')
md.push(`Generated: ${new Date().toISOString().slice(0, 19)}`)
md.push(hasWG ? '' : '(WG fetch unavailable — single-source audit. Auto-fix not safe.)')
md.push('')
md.push('## Summary')
md.push('')
...
const renderEntry = (e) => {
  ...
  if (e.sg)  lines.push(`- WG:  lat=${e.sg.lat}, lon=${e.sg.lon} (sim=${e.sg.sim}, ${e.sg.distFromDb}m from DB)`)
  if (e.osmSgDist != null) lines.push(`- OSM↔WG distance: ${e.osmSgDist}m`)
  lines.push(`- **Reason:** ${e.flagReason}`)
  if (e.flagLevel === 'high' && e.osm && e.sg) {
    const lat = (e.osm.lat + e.sg.lat) / 2
    const lon = (e.osm.lon + e.sg.lon) / 2
    lines.push(`- Suggested fix: lat=${lat.toFixed(7)}, lon=${lon.toFixed(7)} (OSM+WG midpoint)`)
  }
  lines.push('')
  return lines.join('\n')
}
