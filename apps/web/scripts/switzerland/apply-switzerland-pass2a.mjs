// Apply Switzerland Pass 2a website enrichment fra match-candidates.
// Federation-first, kun website i scope (per Thomas-decision session 30 — email/phone droppet).
//
// Run dry-run (default tier=high):
//   node --env-file=.env.local scripts/switzerland/apply-switzerland-pass2a.mjs
// Apply high+medium:
//   node --env-file=.env.local scripts/switzerland/apply-switzerland-pass2a.mjs --tier=both --apply

import { readFileSync, writeFileSync } from 'node:fs'
import { createClient } from '@supabase/supabase-js'

const CANDIDATES_PATH = 'scripts/switzerland/switzerland-match-candidates.json'
const ALLOWED_FIELDS = new Set(['website']) // CH scope: website only
const logPathFor = (tier) => `scripts/switzerland/apply-switzerland-pass2a-${tier}-log.json`
const backupPathFor = (tier, date) =>
  `scripts/switzerland/courses-backup-switzerland-pass2a-${tier}-${date}.json`
...
console.log('=== Step 1/3: Backup alle Switzerland-rows ===')
const backupRows = []
const PAGE = 1000
...
console.log(`Fetched ${backupRows.length} Switzerland-rows fra DB`)
    ...
    JSON.stringify(
      {
        ...
        purpose: `Pre-apply snapshot before Pass 2a (${tierArg}) website enrichment`,
        rows: backupRows,
      },
      null,
      2,
