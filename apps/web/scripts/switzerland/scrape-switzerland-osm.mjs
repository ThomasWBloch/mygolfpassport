// Scrape OSM for Swiss golf clubs via Overpass API.
// Run: node scripts/switzerland/scrape-switzerland-osm.mjs
//
// Mirrors scripts/austria/scrape-austria-osm.mjs — only ISO code, output path
// and locale-specific blocklist substrings differ.

import { writeFileSync, mkdirSync } from 'node:fs'

const OUT_PATH = 'scripts/switzerland/switzerland-clubs-osm.json'

const QUERY = `[out:json][timeout:90];
area["ISO3166-1"="CH"]->.ch;
(node[leisure=golf_course](area.ch);
 way[leisure=golf_course](area.ch);
 relation[leisure=golf_course](area.ch););
out center tags;`
...
// CH-specific blocklist: practice / non-club. Plus French + Italian variants.
const NAME_BLOCKLIST = [
  ...
  'übungsplatz',
  'uebungsplatz',
  'practice', // FR/EN, used for driving ranges in CH
  'campo pratica', // IT
]
...
async function queryServer(server) {
  const resp = await fetch(server, {
    ...
    headers: {
      ...
      'User-Agent': 'mygolfpassport/1.0 (switzerland club scrape; thomasbloch74@gmail.com)',
...
function normalize(el) {
  const tags = el.tags || {}
  // Switzerland has 4 official languages; pick whichever name tag is set.
  const name = (
    tags.name ||
    tags['name:de'] ||
    tags['name:fr'] ||
